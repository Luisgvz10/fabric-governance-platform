...
# Normalizamos "ServicePrincipal" -> "App";

bronze_item_access_users = spark.table("bronze.item_access")

principal_fields = [f.name for f in bronze_item_access_users.schema["principal"].dataType.fields]

user_email_expr = (
    F.col("principal.userDetails.userPrincipalName")
    if "userDetails" in principal_fields
    else F.lit(None).cast("string")
)

principals_from_items = (
    bronze_item_access_users
    .select(
        ...
        F.when(F.col("principal.type") == "User", user_email_expr).alias("email"),
    )
)
